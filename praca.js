// Tablica z ścieżkami do klatek komiksu
const comicFrames = [
    { type: "single", src: "1.png" }, // Pojedyncza klatka
    { type: "double", src1: "2.png", src2: "3.png" }, // Dwie klatki obok siebie
	...
	{ type: "single", src: "21.png",hasTask: true, taskId: "task-3",subtasks: [
            {
                id: "subtask-3-1",
                type: "matrix", // Podpunkt z macierzą
                description: " a) ",
                matrixRows: 2,
                matrixCols: 2,
                correctAnswer: [[6, 12], [7, 3]]
            },
            {
                id: "subtask-3-2",
                type: "text", // Podpunkt z komunikatem
                description: "b) ",
                correctAnswer: "operacja niemożliwa"
            },
			{
                id: "subtask-3-3",
                type: "matrix", 
            ...
            },
			{
                ...
                matrixRows: 3,
                matrixCols: 3,
                correctAnswer: [[6, 2, 0], [-3, 7, 0],[6, -2, 4]]
            }
        ] },
	...
	{ type: "single", src: "23.png" ,hasTask: true, taskId: "task-5",subtasks: [
            ...
            },
			{
                ...
                matrixRows: 3,
                matrixCols: 3,
                correctAnswer: [[2, -2, 1], [2, -2, 1],[6, -6, 3]]
            }
        ] },
	...
	{ type: "single", src: "25.png",hasTask: true, taskId: "task-7",subtasks: [
            {
                ...
                matrixRows: 2,
                matrixCols: 2,
                correctAnswer: [[3/16, 1/4], [-1/16, 1/4]]
            },
            {
                id: "subtask-7-2",
                type: "text", // Podpunkt z komunikatem
                description: "b) ",
                correctAnswer: "nie istnieje"
            },
			{
                id: "subtask-7-3",
                type: "matrix", 
                ...
                matrixRows: 3,
                matrixCols: 3,
                correctAnswer: [[-3, 2, 0], [2, -1, 0],[5, -3, 1]]
            }
        ] },
...
let currentFrameIndex = 0; // Indeks aktualnej klatki
const comicContainer = document.querySelector(".comic-container");
const comicFrameElement = document.getElementById("comic-frame");
...
// Funkcja do aktualizacji klatki
function updateFrame() {
    const currentFrame = comicFrames[currentFrameIndex];

    // Czyścimy kontener przed dodaniem nowej klatki/klatek
    comicContainer.innerHTML = "";

    // Ukrywamy wszystkie zadania i podpowiedzi
    document.querySelectorAll(".task").forEach(task => {
        task.style.display = "none";
    });

    if (currentFrame.type === "single") {
        // Wyświetlamy pojedynczą klatkę
        ...
        // Wyświetlamy dwie klatki obok siebie
    ...
    // Sprawdzamy, czy klatka ma zadanie
    if (currentFrame.hasTask) {
        const task = document.getElementById(currentFrame.taskId);
        if (task) {
            taskSection.style.display = "block";
            task.style.display = "block"; // Pokazujemy zadanie

            // Generowanie podpunktów
            const subtasksContainer = task.querySelector(".subtasks-container");
            console.log("subtasksContainer:", subtasksContainer); // Debugowanie

            if (subtasksContainer) {
                subtasksContainer.innerHTML = ""; // Czyścimy kontener podpunktów

                // Sprawdzamy, czy currentFrame.subtasks istnieje
                if (currentFrame.subtasks) {
                    console.log("Subtasks found:", currentFrame.subtasks); // Debugowanie
                    currentFrame.subtasks.forEach(subtask => {
                        ...
                        console.log("subtaskDiv created:", subtaskDiv); // Debugowanie

                        // Generowanie zawartości podpunktu na podstawie typu
                        if (subtask.type === "matrix") {
                            ...
                            subtaskDiv.appendChild(matrixContainer); // Dodaj kontener do DOM

                            console.log("matrixContainer created with ID:", matrixContainer.id); // Debugowanie

                            // Opóźnij wywołanie createMatrixInputs
                            setTimeout(() => {
                    ...
                    console.error("Brak podpunktów w bieżącym zadaniu!");
                }
            } else {
                console.error("Nie znaleziono subtasks-container w zadaniu!");
            }
        }
    } else {
    ...
    // Aktualizujemy stan przycisków nawigacyjnych
    prevButton.disabled = currentFrameIndex === 0;
    nextButton.disabled = currentFrameIndex === comicFrames.length - 1;

    // Zmieniamy tekst przycisku "Dalej" na "Koniec" na ostatniej klatce
...
updateFrame(); // Inicjalizacja pierwszej klatki
...
function getUserMatrix(rows, cols, containerId) {
    ...
    for (let i = 0; i < rows; i++) {
        ...
        for (let j = 0; j < cols; j++) {
            const value = parseFloat(inputs[i * cols + j].value) || 0; // Domyślna wartość 0, jeśli pole jest puste
            row.push(value);
        }
        matrix.push(row);
    }
...
function createMatrixInputs(rows, cols, containerId) {
    console.log(`Tworzenie macierzy ${rows}x${cols} w kontenerze o ID: "${containerId}"`); // Debugowanie
    const container = document.getElementById(containerId);
    ...
    console.log("Wygenerowano macierz:", container); // Debugowanie
}

// Funkcja do sprawdzania odpowiedzi w quizie
document.querySelectorAll(".submit-answers").forEach(button => {
    button.addEventListener("click", (event) => {
        ...
        let totalQuestions = 0; // Całkowita liczba pytań

        // Sprawdzanie odpowiedzi dla zadań zdefiniowanych w HTML
        ...
        // Sprawdzanie odpowiedzi dla zadań generowanych dynamicznie (np. macierzy)
...
function showHint(event) {
    ...
    const hintButton = event.target; // Przycisk, który został kliknięty
    const taskId = hintButton.id.replace("hint-button-", ""); // Pobierz numer zadania z id przycisku
    const hintMessage = document.getElementById(`hint-message-${taskId}`); // Znajdź odpowiedni komunikat podpowiedzi

    if (hintMessage.style.display === "block") {
        hintMessage.style.display = "none"; // Ukryj podpowiedź, jeśli jest już widoczna
    } else {
        // Ustaw odpowiednią podpowiedź dla danego zadania
        switch (taskId) {
            case "1":
        ...
        hintMessage.style.display = "block"; // Pokaż podpowiedź
    }
}


// Nasłuchiwanie kliknięć na wszystkich przyciskach podpowiedzi
...
// Inicjalizacja przycisków i klatki na starcie
updateFrame();